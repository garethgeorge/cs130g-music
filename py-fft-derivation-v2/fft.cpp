...
int main() {
	Complex samples[] = {Complex(1, 0), Complex(0, 0), Complex(0, 0), Complex(0, 0)};
	Complex result[sizeof(samples) / sizeof(Complex)];
	Complex result2[sizeof(samples) / sizeof(Complex)];

	FFT<4>::run(samples, result);
	IFFT<4>::run(result, result2);
