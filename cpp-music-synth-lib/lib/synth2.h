...
inline extern int seconds(float seconds) {
	return seconds * SAMPLES_PER_SECOND;
}

inline extern int samples(float samples) {
	return (int) samples;
}

inline extern int freq(float freq) {
...
// infinite
template<class T1, class T2>
auto operator + (const Finite<T1>& wave1, const Finite<T2>& wave2) -> Finite<decltype(wave1.wave + wave2.wave)>  {
	return Finite<decltype(wave1.wave + wave2.wave)>(wave1.wave + wave2.wave, std::max(wave1.getDuration(), wave2.getDuration()));
}
...
template<class T1, class T2>
auto operator * (const Finite<T1>& wave1, const Finite<T2>& wave2) -> Finite<decltype(wave1.wave * wave2.wave)> {
	return finiteOf(wave1.wave * wave2.wave, std::min(wave1.getDuration(), wave2.getDuration()));
}
...
struct Envelope : public SoundSource<Envelope<WaveType>> {
	...
	Envelope() { };
	Envelope(const SoundSource<WaveType>& wave, int size) {
		this->size = size;
	...
	float _sample(const Context& context) {
		...
		if (context.time < size) {
			float frac = ((float) context.time) / ((float) size);
			//return frac * wave.sample(context);
		}
		if (context.time > context.duration - size) {
			float frac = ((float) (context.duration - context.time)) / ((float) size);
		}
...
template<class T>
auto envelopeOf(const Finite<T>& wave, float size) {
	return finiteOf(
		envelopeOf(wave.wave, size), wave.getDuration());
}

template<class T>
auto envelopeOf(const SoundSource<T>& wave, float size) {
	return Envelope<T>(wave.get_ref(), size);
}
